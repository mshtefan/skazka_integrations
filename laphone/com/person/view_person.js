...
class ViewPerson extends ViewBase {
    ...
    _prepareTplData() {
        ...
        this._tplData['pointsText'] = 'балл' + ViewBase.getNumEnding(this._tplData['pointsValue'], ['', 'а', 'ов']);

        if (this._tplData['phone']) {
            this._tplData['phone'] = this._tplData['phone'].replace(/(\d{1})(\d{3})(\d{3})(\d{2})(\d{2})/, "+$1($2) $3-$4-$5");
        }
