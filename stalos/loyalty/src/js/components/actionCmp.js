    ...
    .directive('actionsCmp', ['userService', 'actionService', '$rootScope', function (userService, actionService, $rootScope) {
      return {
        ...
        link: function (scope) {
          ...
          var tags_list = [actionService.getTags().profile];

          if (window.config && window.config.actions) {
            tags_list = tags_list.concat(window.config.actions.filter(function (item) {
              return item.enabled
            }).map(function (item) {
              return item.tag
            }))
          ...
          userService.existTags(tags_list).then(function (data) {
            scope.exist.profile = data[0].exist;
            scope.exist_list = data;
          });

          scope.need_to_show = function (action) {
            return scope.exist_list && scope.exist_list.length && scope.exist_list.filter(function (item) {
                return action.tag === item.name && !item.exist
              }).length
          };
          ...
          scope.saveForm = function () {
            ...
            if (validForm(formData)) {
              ...
              var tags = [actionService.getTags().profile, formData.soc_status, formData.job, formData.fam_status, 'Город ' + formData.town, 'Район ' + formData.area];
              data.auth_hash = sp.config().auth_hash;
              sp.send('users.update', data, function (res) {
              ...
              setTimeout(function () {
                scope.$emit('notify', 'Необходимо заполнить все поля!');
                scope.$digest();
              }, 100);
            }
          };
          ...
          scope.isActive = function () {
            return Object.keys(scope.show).length || scope.custom_action ? false : true;
          };
